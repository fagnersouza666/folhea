...
@Component({ selector: 'folhea-login', standalone: true, imports: [ReactiveFormsModule, RouterLink], changeDetection: ChangeDetectionStrategy.OnPush, template: `
  <main class="auth-page"><a routerLink="/" class="brand" aria-label="Voltar para Folhea"><span class="brand-mark">F</span><span>folhea</span></a><section class="auth-card surface"><p class="eyebrow">Bem-vindo de volta</p><h1>Continue sua leitura.</h1><p class="auth-intro">Entre para registrar sua próxima sessão e acompanhar seu progresso.</p><form [formGroup]="form" (ngSubmit)="submit()" novalidate><label class="field" for="email">E-mail<input id="email" type="email" formControlName="email" placeholder="voce@email.com" autocomplete="email" [attr.aria-invalid]="email.invalid && email.touched" [attr.aria-describedby]="email.invalid && email.touched ? 'email-error' : null" />@if (email.invalid && email.touched) { <span id="email-error" class="field-error">Informe um e-mail válido.</span> }</label><label class="field" for="password">Senha<input id="password" type="password" formControlName="password" placeholder="Sua senha" autocomplete="current-password" [attr.aria-invalid]="password.invalid && password.touched" [attr.aria-describedby]="password.invalid && password.touched ? 'password-error' : null" />@if (password.invalid && password.touched) { <span id="password-error" class="field-error">A senha é obrigatória.</span> }</label><button class="button button-primary submit-button" type="submit">Entrar <span aria-hidden="true">→</span></button></form><p class="auth-footnote">Ainda não tem uma conta? <a href="mailto:oi@folhea.com.br">Fale com a gente</a></p></section><p class="auth-legal">Ao continuar, você concorda com nossos <a routerLink="/termos">termos</a> e <a routerLink="/privacidade">política de privacidade</a>.</p></main>
