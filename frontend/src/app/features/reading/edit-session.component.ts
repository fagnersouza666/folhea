    ...
    @if (session(); as currentSession) {
      <div class="form-page"><a routerLink="/app/progresso" class="back-link">← Seu progresso</a><p class="eyebrow">Ajustar registro</p><h1>Editar leitura</h1><p class="intro">Corrija os dados da sessão. Suas métricas serão atualizadas na hora.</p>
        <form class="surface form-card" [formGroup]="form" (ngSubmit)="save()" novalidate>
          <label class="field">Livro<select formControlName="bookId"><option value="" disabled>Escolha um livro</option>@for (book of store.books(); track book.id) { <option [value]="book.id">{{ book.title }}</option> }</select></label>
          <div class="field-row"><label class="field">Páginas<input type="number" min="0" inputmode="numeric" formControlName="pages" [attr.aria-invalid]="pages.invalid && pages.touched" /></label><label class="field">Minutos<input type="number" min="0" inputmode="numeric" formControlName="minutes" [attr.aria-invalid]="minutes.invalid && minutes.touched" /></label></div>
          ...
          <label class="field">Data<input type="date" formControlName="readingDate" /></label>
